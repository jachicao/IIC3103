class StoreHouseController < ApplicationController
    ...
    def clearReception
      ...
      if recepcion
        if recepcion["inventario"]
          inventario = a["inventario"]  # se obtiene el inventario
          total_a_mover = 0
          inventario.each do |i|
            ...
            while count > 0 and total_a_mover < capacidad
              productos = GetProductStockJob.perform_now(a["_id"], i[:sku]) # se buscan todos los productos de un sku
              productos.each do |p|
      ...
      render json: { message: 'Recepcion liberada'}
    end
    ...
    def movetoDespatch(sku, quantity) #cantidad de un sku que hay que llevar a despacho
      ...
      if stock >= quantity
        ...
        render json: { message: 'Movido a despacho', status: 'ok'}
      else
        aux = quantity- stock
        # while stock > 0
        #   almacenamiento.each do |m|
        ...
        #   end
        # end
        render json: { message: 'Falta stock', status: 'not ok', missing: aux}
      end
    end
